...
def _require(inputs: Mapping[str, Any], expected: Mapping[str, Any]) -> None:
    if dict(inputs) != dict(expected):
        raise ValueError("Inputs do not match the declared continuous-HG derivation")


UNIVERSAL_INPUTS: dict[str, dict[str, Any]] = {
...
def _universal(result_id: str, inputs: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    _require(inputs, UNIVERSAL_INPUTS[result_id])
...
def _b16(component: str, inputs: Mapping[str, Any]) -> tuple[Any, dict[str, Any]]:
    ...
    if component == "positivity_equivalence":
        _require(inputs, UNIVERSAL_INPUTS["CHG-B16"])
...
__all__ = ["ASSUMPTION_MODELS", "FOUNDATION_DEPENDENCIES", "FOUNDATION_RULES", "ReplayContinuousHGResult", "UNIVERSAL_INPUTS", "UNIVERSAL_RESULTS", "UNIVERSAL_STEPS", "ValidateContinuousHGAssumptionModel"]
