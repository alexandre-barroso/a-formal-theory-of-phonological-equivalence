...
theorem region20 (l : ℝ) (hl : 0≤l) (hu : l≤1) :
    (∃w,Nonnegative w ∧ SystemHolds system20 w l) ↔ (0 < 1-2*l-l^2) := by
  constructor
  · rintro ⟨⟨w0,w1,w2,w3,w4,w5,w6,w7,w8⟩,hw,hs⟩
    rcases hw with ⟨h0,h1,h2,h3,h4,h5,h6,h7,h8⟩
    norm_num [SystemHolds,system20,evalDelta,dotInt,values] at hs
    ...
    have hc : MutualCounterfeedingRegion.Selects true l ⟨w0,w3,w2,w1⟩ := by
      norm_num [MutualCounterfeedingRegion.Selects];(repeat' constructor) <;> linarith
    exact MutualCounterfeedingRegion.necessary_carried l hl _ hn hc
  · intro h
    refine ⟨⟨1+l,(1-l^2)/2,(3+2*l+l^2)/4,1,0,0,0,0,0⟩,?_,?_⟩
    · norm_num [Nonnegative] <;> (repeat' constructor) <;> nlinarith [sq_nonneg l]
    · norm_num [SystemHolds,system20,evalDelta,dotInt,values]
